/* practice-session.js — core spelling loop with optional custom keyboard */
(function () {
  'use strict';
  ...
  SR.PracticeSession = class PracticeSession {
    constructor(config, navigate, container) {
      ...
      this._keyboard        = null;    // SpellKeyboard instance (or null)
      this._inputEl         = null;    // cached for fast key insertion
      this._submitEl        = null;    // cached for fast enable/disable
      this._kbToggleHandler = null;    // live-switch event listener ref
    }
    ...
    async mount() {
      ...
      /* Listen for live keyboard-mode switch from Settings */
      this._kbToggleHandler = (e) => { if (!this._destroyed) this._onKeyboardToggle(e.detail); };
      document.addEventListener('spellright:appkeyboard', this._kbToggleHandler);
    ...
    _render() {
      const useAppKb = SR.store.state.settings.useAppKeyboard;
      ...
      this.container.innerHTML = `
        <div class="practice-session" id="ps-root">
          ...
          <div class="practice-input-area">
            <div class="practice-input-row">
              <input
                ...
                type="text"
                ${useAppKb ? 'inputmode="none"' : 'inputmode="text" enterkeyhint="send"'}
                autocomplete="off"
                autocorrect="off"
                autocapitalize="none"
                spellcheck="false"
          ...
          <!-- Custom keyboard mounts here (hidden when useAppKeyboard is OFF) -->
          <div id="ps-keyboard-mount" class="practice-keyboard-wrap"
            ${!useAppKb ? 'style="display:none"' : ''}></div>

        </div>`;

      this._wireInputEvents();
      document.getElementById('ps-quit').addEventListener('click', () => this._quit());
    }

    /* ── Mount/unmount custom keyboard ───────────────────────────────────── */

    _mountKeyboard() {
      if (!SR.store.state.settings.useAppKeyboard) return;

      const mount = document.getElementById('ps-keyboard-mount');
      if (!mount) return;

      /* Cache refs once — reused on every keystroke for zero-latency input */
      this._inputEl  = document.getElementById('ps-input');
      this._submitEl = document.getElementById('ps-submit');
    ...
    /* ── Live keyboard mode switch (fired by Settings toggle) ────────────── */

    _onKeyboardToggle(useAppKb) {
      const mount = document.getElementById('ps-keyboard-mount');
      const input = this._inputEl || document.getElementById('ps-input');
      if (!input) return;
      ...
      if (useAppKb) {
        ...
        if (mount) {
          mount.style.display = '';
          /* Mount keyboard if it doesn't exist yet */
          if (!this._keyboard) {
            this._inputEl  = input;
            this._submitEl = document.getElementById('ps-submit');
            this._keyboard = new SR.SpellKeyboard({
        ...
        /* Focus opens the native keyboard on mobile */
        setTimeout(() => { if (!this._destroyed) input.focus(); }, 60);
      }
    }
    ...
    _wireInputEvents() {
      ...
      /* Physical keyboard Enter / mobile "Go" key */
      ...
      submit.addEventListener('click', () => {
        SR.helpers.haptic.submit();
        if (!this._locked) this._handleSubmit();
      });
    }

    /* ── Custom keyboard character insertion ─────────────────────────────── */
    ...
    _showQuestion() {
      ...
      const input  = this._inputEl || document.getElementById('ps-input');
      const submit = this._submitEl || document.getElementById('ps-submit');
      ...
      if (input) {
        ...
        if (SR.store.state.settings.useAppKeyboard) input.setAttribute('readonly', '');
      }
      ...
      /* Auto-focus: if using native keyboard, open it; if custom, keep input focused for Enter */
      if (!SR.store.state.settings.useAppKeyboard && input) {
        setTimeout(() => { if (!this._destroyed) input.focus(); }, 350);
      }
    }
    ...
    async _playWord(word) {
      ...
      if (this._destroyed) return; // session may have ended during async speak
    ...
    _handleSubmit() {
      ...
      if (isCorrect) {
        SR.store.markCorrect(expected);
        this.correct.push(expected);
        SR.helpers.haptic.correct();
      } else {
        SR.store.markWrong(expected);
        this.wrong.push(expected);
        SR.helpers.haptic.wrong();
      }
        ...
        <div class="screen fade-scale-in">
          ...
          <div class="results-grid">
            ...
            <div class="card" style="border-color:var(--success-bg)">
              <div class="value" style="color:var(--success)">${correct}</div><p class="text-caption">Correct</p>
            </div>
            <div class="card" style="border-color:var(--danger-bg)">
              <div class="value" style="color:var(--danger)">${wrong}</div><p class="text-caption">Wrong</p>
            </div>
          </div>
          ${wrong > 0
      ...
      if (wrongBtn) {
        wrongBtn.addEventListener('click', () => {
          ...
          this.navigate('practice-session', {
            questions: doubled.length, wordOverride: doubled,
            wordSource:'wrong', wordLength:'all',
            mode: this.cfg.mode, timer: this.cfg.timer, hintsEnabled: this.cfg.hintsEnabled,
          });
        });
      }
    }
    ...
    _quit() {
      ...
      document.getElementById('q-yes').addEventListener('click', () => {
        SR.Modal.close(); this.destroy(); this.navigate('home');
      });
    ...
    destroy() {
      ...
      if (this._kbToggleHandler) {
        document.removeEventListener('spellright:appkeyboard', this._kbToggleHandler);
        this._kbToggleHandler = null;
      }
    }
  };
})();
