/* practice-session.js — the core spelling loop */
(function () {
  'use strict';
  ...
  SR.PracticeSession = class PracticeSession {
    ...
    _showQuestion() {
      ...
      // Auto-play with short delay so stage transition settles
      ...
      // Auto-focus input
      setTimeout(() => { if (!this._destroyed && input) input.focus(); }, 350);
    }
    ...
    async _playWord(word) {
      ...
      // Re-query in case DOM changed while awaiting
    ...
    _startTimer() {
      ...
      this._timerInterval = setInterval(() => {
        ...
        if (this._timerRemaining <= 0) {
          ...
          if (typed) {
            // User typed something — evaluate it normally
            this._handleSubmit();
          } else {
            // Nothing typed — mark wrong and show feedback
            this._locked = true;
            const word = this.queue[this.index];
        ...
        <div class="screen fade-scale-in">
          ...
          <div class="results-grid">
            <div class="card">
              ...
              <p class="text-caption">Questions</p>
            </div>
